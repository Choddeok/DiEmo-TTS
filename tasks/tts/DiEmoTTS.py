...
class ExpressiveFS2Task(FastSpeechTask):
    ...
    def forward(self, sample, infer=False, *args, **kwargs):
        ...
        if not infer:
            ...
            epoch = (self.global_step * hparams["max_sentences"]) // 12155 # 12155 is the total number of training samples in the dataset
            nepoch = math.ceil((160000 * hparams["max_sentences"]) / 12155) # Calculate how many epochs are needed to process 160,000 steps
            dino_loss = DINOLoss(
                hparams["num_emo"],
